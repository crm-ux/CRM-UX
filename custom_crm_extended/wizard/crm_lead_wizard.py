...
class CrmLeadWizard(models.TransientModel):
    ...
    x_requirement_date = fields.Date(string='Exact Date of Requirement')
    ...
    show_contact_error = fields.Boolean(default=False)
    ...
    # ── ONCHANGE: Fetch existing customer details ──────────────────
    ...
    def _onchange_customer_type(self):
        if not self.x_customer_type.startswith('existing'):
    ...
    def _check_step2(self):
        return bool(self.email_from or self.phone or self.x_mobile)

    def action_goto_3(self):
        if not self._check_step2():
            self.show_contact_error = True
            self.step = 2
        else:
            self.show_contact_error = False
            self.step = 3
        return self._reopen()
    ...
    def action_next(self):
        if self.step == 2 and not self._check_step2():
            self.show_contact_error = True
            return self._reopen()
        self.show_contact_error = False
        self.step += 1
    ...
    def action_save_lead(self):
        """Final step — create the crm.lead record."""
        self._validate_step()
        vals = {
            ...
            'priority': {'low': '0', 'medium': '1', 'high': '2'}.get(self.x_lead_priority or 'medium', '0'),
        }
        lead = self.env['crm.lead'].create(vals)
        # Redirect to CRM pipeline list showing the new lead
        return {
            'type': 'ir.actions.act_window',
            'name': 'Pipeline',
            'res_model': 'crm.lead',
